...
module warpgate::swap_test {
    use std::signer;
    use test_coin::test_coins::{Self, TestWARP, TestMUSD, TestUSDC, TestMOVE, TestAPT};
    ...
    fun test_add_liquidity(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, bob, 100 * pow(10, 8));
        test_coins::register_and_mint<TestWARP>(&coin_owner, alice, 100 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, alice, 100 * pow(10, 8));
        ...
        // bob provider liquidity for 5:10 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(bob, bob_liquidity_x, bob_liquidity_y, 0, 0);
        router::add_liquidity<TestWARP, TestMUSD>(alice, alice_liquidity_x, alice_liquidity_y, 0, 0);

        let (balance_y, balance_x) = swap::token_balances<TestMUSD, TestWARP>();
        let (reserve_y, reserve_x, _) = swap::token_reserves<TestMUSD, TestWARP>();
        let resource_account_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(resource_account));
        let bob_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(bob));
        let alice_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(alice));
    ...
    fun test_add_liquidity_with_less_x_ratio(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, bob, 200 * pow(10, 8));
        ...
        let bob_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(bob));

        let bob_add_liquidity_x = 1 * pow(10, 8);
        let bob_add_liquidity_y = 5 * pow(10, 8);
        router::add_liquidity<TestWARP, TestMUSD>(bob, bob_add_liquidity_x, bob_add_liquidity_y, 0, 0);
        ...
        let bob_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(bob));
        let bob_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(bob));
        let resource_account_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(resource_account));
    ...
    fun test_add_liquidity_with_less_x_ratio_and_less_than_y_min(
        ...
        // bob provider liquidity for 5:10 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(bob, initial_reserve_x, initial_reserve_y, 0, 0);

        let bob_add_liquidity_x = 1 * pow(10, 8);
        let bob_add_liquidity_y = 5 * pow(10, 8);
        router::add_liquidity<TestWARP, TestMUSD>(bob, bob_add_liquidity_x, bob_add_liquidity_y, 0, 4 * pow(10, 8));
    }
    ...
    fun test_add_liquidity_with_less_y_ratio_and_less_than_x_min(
        ...
        let bob_add_liquidity_x = 5 * pow(10, 8);
        let bob_add_liquidity_y = 4 * pow(10, 8);
        router::add_liquidity<TestWARP, TestMUSD>(bob, bob_add_liquidity_x, bob_add_liquidity_y, 5 * pow(10, 8), 0);
    }
    ...
    fun test_remove_liquidity(
        ...
        // bob provider liquidity for 5:10 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(bob, bob_add_liquidity_x, bob_add_liquidity_y, 0, 0);
        router::add_liquidity<TestWARP, TestMUSD>(alice, alice_add_liquidity_x, alice_add_liquidity_y, 0, 0);
        ...
        let alice_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(alice));
        let bob_token_x_before_balance = coin::balance<TestWARP>(signer::address_of(bob));
        let bob_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(bob));

        router::remove_liquidity<TestWARP, TestMUSD>(bob, (bob_suppose_lp_balance as u64), 0, 0);
        ...
        router::remove_liquidity<TestWARP, TestMUSD>(alice, (alice_suppose_lp_balance as u64), 0, 0);
        ...
        let alice_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(alice));
        let bob_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(bob));
        let alice_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(alice));
        let alice_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(alice));
        let bob_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(bob));
        let bob_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(bob));
        let (balance_y, balance_x) = swap::token_balances<TestMUSD, TestWARP>();
        let (reserve_y, reserve_x, _) = swap::token_reserves<TestMUSD, TestWARP>();
        let total_supply = std::option::get_with_default(
            &coin::supply<LPToken<TestMUSD, TestWARP>>(),
            0u128
        );
    ...
    fun test_remove_liquidity_with_more_user(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user1, 100 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user2, 100 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user3, 100 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user4, 100 * pow(10, 8));
        ...
        // bob provider liquidity for 5:10 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(user1, user1_add_liquidity_x, user1_add_liquidity_y, 0, 0);
        router::add_liquidity<TestWARP, TestMUSD>(user2, user2_add_liquidity_x, user2_add_liquidity_y, 0, 0);
        router::add_liquidity<TestWARP, TestMUSD>(user3, user3_add_liquidity_x, user3_add_liquidity_y, 0, 0);
        router::add_liquidity<TestWARP, TestMUSD>(user4, user4_add_liquidity_x, user4_add_liquidity_y, 0, 0);
        ...
        let user1_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user1));
        let user2_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user2));
        let user3_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user3));
        let user4_lp_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user4));
        ...
        let user1_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user1));
        let user2_token_x_before_balance = coin::balance<TestWARP>(signer::address_of(user2));
        let user2_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user2));
        let user3_token_x_before_balance = coin::balance<TestWARP>(signer::address_of(user3));
        let user3_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user3));
        let user4_token_x_before_balance = coin::balance<TestWARP>(signer::address_of(user4));
        let user4_token_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user4));

        router::remove_liquidity<TestWARP, TestMUSD>(user1, (user1_suppose_lp_balance as u64), 0, 0);
        ...
        router::remove_liquidity<TestWARP, TestMUSD>(user2, (user2_suppose_lp_balance as u64), 0, 0);
        ...
        router::remove_liquidity<TestWARP, TestMUSD>(user3, (user3_suppose_lp_balance as u64), 0, 0);
        ...
        router::remove_liquidity<TestWARP, TestMUSD>(user4, (user4_suppose_lp_balance as u64), 0, 0);
        ...
        let user1_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user1));
        let user2_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user2));
        let user3_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user3));
        let user4_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(user4));

        let user1_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(user1));
        let user1_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user1));
        let user2_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(user2));
        let user2_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user2));
        let user3_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(user3));
        let user3_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user3));
        let user4_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(user4));
        let user4_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user4));

        let (balance_y, balance_x) = swap::token_balances<TestMUSD, TestWARP>();
        let (reserve_y, reserve_x, _) = swap::token_reserves<TestMUSD, TestWARP>();
        let total_supply = swap::total_lp_supply<TestMUSD, TestWARP>();
    ...
    fun test_remove_liquidity_imbalance(
        ...
        router::remove_liquidity<TestWARP, TestMUSD>(bob, bob_lp_balance, 0, 0);
        // expect the small amount will result one of the amount to be zero and unable to remove liquidity
        router::remove_liquidity<TestWARP, TestMUSD>(alice, alice_lp_balance, 0, 0);
    }
    ...
    fun test_swap_exact_input(
        ...
        router::swap_exact_input<TestWARP, TestMUSD>(alice, input_x, 0);
        ...
        swap::withdraw_fee<TestWARP, TestMUSD>(treasury);
        let treasury_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(treasury));
        router::remove_liquidity<TestWARP, TestMUSD>(treasury, (suppose_fee_amount as u64), 0, 0);
        let treasury_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(treasury));
        let treasury_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(treasury));
    ...
    fun test_withdraw_fee_noauth(
        ...
        // bob provider liquidity for 5:10 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(bob, initial_reserve_x, initial_reserve_y, 0, 0);
        if(swap_utils::sort_token_type<TestWARP, TestMUSD>()){
            swap::check_or_register_coin_store<LPToken<TestWARP, TestMUSD>>(treasury);
        }else{
            swap::check_or_register_coin_store<LPToken<TestMUSD, TestWARP>>(treasury);
        };
        let bob_suppose_lp_balance = math::sqrt(((initial_reserve_x as u128) * (initial_reserve_y as u128))) - MINIMUM_LIQUIDITY;
        let suppose_total_supply = bob_suppose_lp_balance + MINIMUM_LIQUIDITY;
        ...
        swap::withdraw_fee_noauth<TestWARP, TestMUSD>();
        let treasury_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(treasury));
        router::remove_liquidity<TestWARP, TestMUSD>(treasury, (suppose_fee_amount as u64), 0, 0);
        let treasury_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(treasury));
        let treasury_token_y_after_balance = coin::balance<TestMUSD>(signer::address_of(treasury));
    ...
    fun test_swap_exact_input_overflow(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, bob, MAX_U64);
    ...
    fun test_swap_exact_input_with_not_enough_liquidity(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, bob, 1000 * pow(10, 8));
    ...
    fun test_swap_exact_input_under_min_output(
        ...
        let output_y = calc_output_using_input(input_x, initial_reserve_x, initial_reserve_y);
        router::swap_exact_input<TestWARP, TestMUSD>(alice, input_x, ((output_y + 1) as u64));
    }
    ...
    fun test_swap_exact_output(
        ...
        router::swap_exact_output<TestWARP, TestMUSD>(alice, output_y, input_x_max);
    ...
    fun test_swap_exact_output_excceed_max_input(
        ...
        let input_x = calc_input_using_output(output_y, initial_reserve_x, initial_reserve_y);
        router::swap_exact_output<TestWARP, TestMUSD>(alice, output_y, ((input_x - 1) as u64));
    }
    ...
    fun test_swap_x_to_exact_y_direct_external(
        ...
        let x_in_amount = router::get_amount_in<TestWARP, TestMUSD>(output_y);
        assert!(x_in_amount == (input_x as u64), 102);

        let input_x_coin = coin::withdraw(alice, (input_x as u64));

        let (x_out, y_out) =  router::swap_x_to_exact_y_direct_external<TestWARP, TestMUSD>(input_x_coin, output_y);

        assert!(coin::value(&x_out) == 0, 101);
        assert!(coin::value(&y_out) == output_y, 100);
        coin::register<TestMUSD>(alice);
        coin::deposit<TestWARP>(alice_addr, x_out);
        coin::deposit<TestMUSD>(alice_addr, y_out);

        let alice_token_x_after_balance = coin::balance<TestWARP>(alice_addr);
        let alice_token_y_after_balance = coin::balance<TestMUSD>(alice_addr);
    ...
    fun test_get_amount_in(
        ...
        let y_in_amount = router::get_amount_in<TestMUSD, TestWARP>(output_x);
    ...
    fun test_swap_exact_input_doublehop(
        ...
        // bob provider liquidity for 1:2 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(bob, initial_reserve_xy_x, initial_reserve_xy_y, 0, 0);
        ...
        // bob provider liquidity for 2:1 USDC-MUSD
        router::add_liquidity<TestUSDC, TestMUSD>(bob, initial_reserve_yz_z, initial_reserve_yz_y, 0, 0);
        ...
        router::swap_exact_input_doublehop<TestWARP, TestMUSD, TestUSDC>(alice, input_x, 0);
        ...
        let (reserve_xy_y, reserve_xy_x, _) = swap::token_reserves<TestMUSD, TestWARP>();
        let (reserve_yz_y, reserve_yz_z, _) = swap::token_reserves<TestMUSD, TestUSDC>();
        assert!((alice_token_x_before_balance - alice_token_x_after_balance) == input_x, 99);
        ...
        let bob_token_xy_y_before_balance = coin::balance<TestMUSD>(signer::address_of(bob));

        router::remove_liquidity<TestWARP, TestMUSD>(bob, (bob_suppose_xy_lp_balance as u64), 0, 0);

        let bob_token_xy_x_after_balance = coin::balance<TestWARP>(signer::address_of(bob));
        let bob_token_xy_y_after_balance = coin::balance<TestMUSD>(signer::address_of(bob));
        ...
        let bob_token_yz_y_before_balance = coin::balance<TestMUSD>(signer::address_of(bob));
        let bob_token_yz_z_before_balance = coin::balance<TestUSDC>(signer::address_of(bob));

        router::remove_liquidity<TestMUSD, TestUSDC>(bob, (bob_suppose_yz_lp_balance as u64), 0, 0);

        let bob_token_yz_y_after_balance = coin::balance<TestMUSD>(signer::address_of(bob));
        ...
        swap::withdraw_fee<TestWARP, TestMUSD>(treasury);
        let treasury_xy_lp_after_balance = coin::balance<LPToken<TestMUSD, TestWARP>>(signer::address_of(treasury));
        router::remove_liquidity<TestWARP, TestMUSD>(treasury, (suppose_xy_fee_amount as u64), 0, 0);
        let treasury_token_xy_x_after_balance = coin::balance<TestWARP>(signer::address_of(treasury));
        let treasury_token_xy_y_after_balance = coin::balance<TestMUSD>(signer::address_of(treasury));
        ...
        swap::withdraw_fee<TestMUSD, TestUSDC>(treasury);
        let treasury_yz_lp_after_balance = coin::balance<LPToken<TestMUSD, TestUSDC>>(signer::address_of(treasury));
        router::remove_liquidity<TestMUSD, TestUSDC>(treasury, (suppose_yz_fee_amount as u64), 0, 0);
        let treasury_token_yz_y_after_balance = coin::balance<TestMUSD>(signer::address_of(treasury));
    ...
    fun swap_exact_output_doublehop(
        ...
        router::swap_exact_output_doublehop<TestWARP, TestMUSD, TestUSDC>(alice, output_z, 1 * pow(10, 8));
    ...
    fun test_swap_exact_input_triplehop(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, bob, 200 * pow(10, 8));
        test_coins::register_and_mint<TestUSDC>(&coin_owner, bob, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMOVE>(&coin_owner, bob, 200 * pow(10, 8));
        test_coins::register_and_mint<TestWARP>(&coin_owner, alice, 100 * pow(10, 8));
        ...
        let bob_suppose_yz_lp_balance = math::sqrt(((initial_reserve_yz_y as u128) * (initial_reserve_yz_z as u128))) - MINIMUM_LIQUIDITY;
        let suppose_yz_total_supply = bob_suppose_yz_lp_balance + MINIMUM_LIQUIDITY;
        // bob provider liquidity for 2:3 USDC-MUSD
        router::add_liquidity<TestUSDC, TestMOVE>(bob, initial_reserve_za_z, initial_reserve_za_a, 0, 0);
        ...
        router::swap_exact_input_triplehop<TestWARP, TestMUSD, TestUSDC, TestMOVE>(alice, input_x, 0);

        let alice_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(alice));
        let alice_token_a_after_balance = coin::balance<TestMOVE>(signer::address_of(alice));
        ...
        let (reserve_xy_y, reserve_xy_x, _) = swap::token_reserves<TestMUSD, TestWARP>();
        let (reserve_yz_y, reserve_yz_z, _) = swap::token_reserves<TestMUSD, TestUSDC>();
        let (reserve_za_a, reserve_za_z, _) = swap::token_reserves<TestMOVE, TestUSDC>();
        assert!((alice_token_x_before_balance - alice_token_x_after_balance) == input_x, 99);
        ...
        let bob_token_za_a_before_balance = coin::balance<TestMOVE>(signer::address_of(bob));

        router::remove_liquidity<TestUSDC, TestMOVE>(bob, (bob_suppose_za_lp_balance as u64), 0, 0);

        let bob_token_za_z_after_balance = coin::balance<TestUSDC>(signer::address_of(bob));
        let bob_token_za_a_after_balance = coin::balance<TestMOVE>(signer::address_of(bob));
        ...
        swap::withdraw_fee<TestUSDC, TestMOVE>(treasury);
        let treasury_za_lp_after_balance = coin::balance<LPToken<TestMOVE, TestUSDC>>(signer::address_of(treasury));
        router::remove_liquidity<TestMOVE, TestUSDC>(treasury, (suppose_za_fee_amount as u64), 0, 0);
        let treasury_token_za_z_after_balance = coin::balance<TestUSDC>(signer::address_of(treasury));
        let treasury_token_za_a_after_balance = coin::balance<TestMOVE>(signer::address_of(treasury));
    ...
    fun test_swap_exact_output_triplehop(
        ...
        router::swap_exact_output_triplehop<TestWARP, TestMUSD, TestUSDC, TestMOVE>(alice, output_a, 1 * pow(10, 8));
    ...
    fun test_swap_exact_input_triplehop_with_multi_liquidity(
        ...
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user1, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user2, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user3, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMUSD>(&coin_owner, user4, 200 * pow(10, 8));
        ...
        test_coins::register_and_mint<TestMOVE>(&coin_owner, user1, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMOVE>(&coin_owner, user2, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMOVE>(&coin_owner, user3, 200 * pow(10, 8));
        test_coins::register_and_mint<TestMOVE>(&coin_owner, user4, 200 * pow(10, 8));
        test_coins::register_and_mint<TestWARP>(&coin_owner, alice, 100 * pow(10, 8));
        ...
        // bob provider liquidity for 1:2 WARP-MUSD
        router::add_liquidity<TestWARP, TestMUSD>(user1, user1_add_liquidity_xy_x, user1_add_liquidity_xy_y, 0, 0);
        ...
        router::add_liquidity<TestWARP, TestMUSD>(user2, user2_add_liquidity_xy_x, user2_add_liquidity_xy_y, 0, 0);
        ...
        router::add_liquidity<TestWARP, TestMUSD>(user3, user3_add_liquidity_xy_x, user3_add_liquidity_xy_y, 0, 0);
        ...
        router::add_liquidity<TestWARP, TestMUSD>(user4, user4_add_liquidity_xy_x, user4_add_liquidity_xy_y, 0, 0);
        ...
        // bob provider liquidity for 2:1 USDC-MUSD
        router::add_liquidity<TestMUSD, TestUSDC>(user1, user1_add_liquidity_yz_y, user1_add_liquidity_yz_z, 0, 0);
        let suppose_reserve_yz_y = user1_add_liquidity_yz_y;
        let suppose_reserve_yz_z = user1_add_liquidity_yz_z;
        router::add_liquidity<TestMUSD, TestUSDC>(user2, user2_add_liquidity_yz_y, user2_add_liquidity_yz_z, 0, 0);
        ...
        // bob provider liquidity for 2:3 USDC-TestMOVE
        router::add_liquidity<TestUSDC, TestMOVE>(user1, user1_add_liquidity_za_z, user1_add_liquidity_za_a, 0, 0);
        let suppose_reserve_za_z = user1_add_liquidity_za_z;
        let suppose_reserve_za_a = user1_add_liquidity_za_a;
        router::add_liquidity<TestUSDC, TestMOVE>(user2, user2_add_liquidity_za_z, user2_add_liquidity_za_a, 0, 0);
        ...
        let alice_token_a_before_balance = coin::balance<TestMOVE>(signer::address_of(alice));

        router::swap_exact_input_triplehop<TestWARP, TestMUSD, TestUSDC, TestMOVE>(alice, input_x, 0);

        alice_token_x_after_balance = coin::balance<TestWARP>(signer::address_of(alice));
        alice_token_a_after_balance = coin::balance<TestMOVE>(signer::address_of(alice));
        ...
        (reserve_xy_y, reserve_xy_x, _) = swap::token_reserves<TestMUSD, TestWARP>();
        (reserve_yz_y, reserve_yz_z, _) = swap::token_reserves<TestMUSD, TestUSDC>();
        (reserve_za_a, reserve_za_z, _) = swap::token_reserves<TestMOVE, TestUSDC>();
        assert!((alice_token_x_before_balance - alice_token_x_after_balance) == input_x, 99);
        ...
        let user1_token_xy_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user1));

        router::remove_liquidity<TestWARP, TestMUSD>(user1, (user1_suppose_xy_lp_balance as u64), 0, 0);

        let user1_token_xy_x_after_balance = coin::balance<TestWARP>(signer::address_of(user1));
        let user1_token_xy_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user1));
        ...
        let user2_token_xy_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user2));

        router::remove_liquidity<TestWARP, TestMUSD>(user2, (user2_suppose_xy_lp_balance as u64), 0, 0);

        let user2_token_xy_x_after_balance = coin::balance<TestWARP>(signer::address_of(user2));
        let user2_token_xy_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user2));
        ...
        let user3_token_xy_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user3));

        router::remove_liquidity<TestWARP, TestMUSD>(user3, (user3_suppose_xy_lp_balance as u64), 0, 0);

        let user3_token_xy_x_after_balance = coin::balance<TestWARP>(signer::address_of(user3));
        let user3_token_xy_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user3));
        ...
        let user4_token_xy_y_before_balance = coin::balance<TestMUSD>(signer::address_of(user4));

        router::remove_liquidity<TestWARP, TestMUSD>(user4, (user4_suppose_xy_lp_balance as u64), 0, 0);

        let user4_token_xy_x_after_balance = coin::balance<TestWARP>(signer::address_of(user4));
        let user4_token_xy_y_after_balance = coin::balance<TestMUSD>(signer::address_of(user4));
    ...
    fun test_swap_exact_input_quadruplehop(
        ...
        router::add_liquidity<TestMUSD, TestUSDC>(bob, initial_reserve_yz_y, initial_reserve_yz_z, 0, 0);
    
        router::add_liquidity<TestUSDC, TestMOVE>(bob, initial_reserve_za_z, initial_reserve_za_a, 0, 0);
        
        router::add_liquidity<TestMOVE, TestAPT>(bob, initial_reserve_ab_a, initial_reserve_ab_b, 0, 0);

        let alice_token_x_before_balance = coin::balance<TestWARP>(signer::address_of(alice));

        router::swap_exact_input_quadruplehop<TestWARP, TestMUSD, TestUSDC, TestMOVE, TestAPT>(alice, input_x, 0);
        ...
        let (reserve_xy_x, reserve_xy_y) = get_token_reserves<TestWARP, TestMUSD>();
        let (reserve_yz_y, reserve_yz_z) = get_token_reserves<TestMUSD, TestUSDC>();
        let (reserve_za_z, reserve_za_a) = get_token_reserves<TestUSDC, TestMOVE>();
        let (reserve_ab_a, reserve_ab_b) = get_token_reserves<TestMOVE, TestAPT>();
    ...
    fun test_swap_exact_output_quadruplehop(
        ...
        router::swap_exact_output_quadruplehop<TestWARP, TestMUSD, TestUSDC, TestMOVE, TestAPT>(alice, output_b, 100 * pow(10, 8));
